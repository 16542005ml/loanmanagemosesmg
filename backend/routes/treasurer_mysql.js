...
router.get('/stats', async (req, res) => {
  try {
    const admin = requireAdmin(req, res);
    if (!admin) return;
    const scope = req.query.scope === 'personal' ? 'personal' : 'chama';
    const [loanRows] = await sequelize.query(
      `SELECT COALESCE(SUM(amount), 0) AS totalLoans,
              COALESCE(SUM(status = 'Overdue'), 0) AS overdueLoans
       FROM loans WHERE status != 'Settled' AND admin_id = :adminId`,
      { type: sequelize.QueryTypes.SELECT, replacements: { adminId: admin.id } }
    );
    ...
    const overdueLoans = Number(lR?.overdueLoans || 0);
    const base = totalLoans + totalSavings;
    const healthScore = base === 0 ? 100 : Math.max(0, Math.min(100, Math.round(((totalSavings - totalLoans) / base) * 50 + 50)));
    const riskFactor = healthScore < 40 ? 'High Risk' : healthScore < 65 ? 'Watch' : 'Stable';

    return ok(res, { scope, totalLoans, totalSavings, overdueLoans, healthScore, riskFactor });
  } catch (e) {
