...
describe('Auth API', () => {
  const authTestEmail = `testadmin_login_${Date.now()}@example.com`;

  beforeAll(async () => {
    const res = await post('/api/auth/register', {
      adminName: 'Login Test Admin',
      adminEmail: authTestEmail,
...
describe('Protected Endpoints', () => {
  ...
  test.each(['/api/checkins/me', '/api/badges/me', '/api/savings-goals/me'])('%s requires member auth', async (path) => {
    const res = await get(path);
    expect(res.status).toBe(401);
    expect(res.body.status).toBe('fail');
  });

  test('GET /api/loans/payoff/:memberId requires member auth', async () => {
    const res = await get('/api/loans/payoff/1');
    expect(res.status).toBe(401);
    expect(res.body.status).toBe('fail');
  });
});
